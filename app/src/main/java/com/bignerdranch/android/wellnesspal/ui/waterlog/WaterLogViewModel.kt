...
class WaterLogViewModel : ViewModel(){
    ...
    fun writeNewWaterLog(oz:String) {
        val date = getDateAndTime()
        val waterLog = WaterLog("water", date, oz )
            ...
            override fun onDataChange(snapshot: DataSnapshot) {
                if (snapshot != null) {
                    for (child in snapshot.children) {
                        ...
                        val log = child.getValue<WaterLog>()
                        val currDate = getCurrentDate()
                        if (log != null) {
                            if (log.date?.contains(currDate) == true) {
                                val amt = log.ozWater.toString()
                                Log.d(TAG, "oz of water: $amt")
                                totalOz += log.ozWater?.toInt() ?: 0
                            }

                        }
                    }
                    totalLogsCurrDateData.value = totalOz
                }
            }
